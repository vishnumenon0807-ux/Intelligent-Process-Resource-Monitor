...
def load_anomaly(conn, anomaly_id):
    with conn.cursor() as cur:
        cur.execute(
            ...
            SELECT a.id, a.snapshot_id, a.timestamp, a.anomaly_score,
                   ...
                   s.cpu_percent, s.ram_percent, s.gpu_percent
            FROM anomalies a
    ...
    keys = ["id", "snapshot_id", "timestamp", "score", "severity",
            "triggered", "sys_cpu", "sys_ram", "sys_gpu"]
...
def explain_anomaly(conn, anomaly_id, verbose=False):
    ...
    if verbose:
        ...
        print(f"  system: cpu {sys_cpu:.1f}%  ram {anomaly['sys_ram']:.1f}%  "
    ...
    if share < DOMINANCE_THRESHOLD:
        ...
        result = {
            ...
            "explanation": (
                ...
                f"{dominant['name']}, explains only {share:.0f}% of CPU load. "
    ...
    contributing = {
        t["name"]: round(t["cpu_percent"], 2)
        for t in trees[1:4] if t["cpu_percent"] > 1.0
    }
